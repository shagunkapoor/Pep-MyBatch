...
public class BinaryTree {
    ...
    public ArrayList<Integer> leftView(Node root) {
        return leftViewHelper(root, 1);
    }

    static int maxLevel = 0;

    private ArrayList<Integer> leftViewHelper(Node node, int level) {
        if (node == null) {
            return new ArrayList<>();
        }
        ...
        ArrayList<Integer> leftRes = leftViewHelper(node.left, level + 1);
        res.addAll(leftRes);
        ArrayList<Integer> rightRes = leftViewHelper(node.right, level + 1);
        res.addAll(rightRes);
        return res;
    }
}
